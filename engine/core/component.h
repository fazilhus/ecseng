...
namespace Ecs {
	...
	public:
		...
		ComponentID GetComponentType() const {
			...
			assert(this->m_componentTypes.find(tname) != this->m_componentTypes.end() && "component not registered registered");
		...
		bool HasComponent(EntityID e) const {
			return this->GetComponentPool().Has(e);
		}

		template <typename T>
		const T& GetComponent(EntityID e) const {
			return this->GetComponentPool().Get(e);
		}
		...
		void AddComponent(EntityID e, Args&& ...args) {
			this->GetComponentPool().Add(e, std::forward(args)...);
		}

		template <typename T>
		void RemoveComponent(EntityID e) {
			this->GetComponentPool().Remove(e);
		}
	...
	private:
		...
		void RegisterComponents(ComponentGroup<Component...>) {
			([&]() {
				...
				this->m_components[tname] = ComponentPool<Component>();
